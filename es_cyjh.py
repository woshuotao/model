...
def _first_query():
    ...
    try:
        ...
        "size": 10000,
        "query": {
        "multi_match": {"query":"ots.changyoujinhua.com",
        "fields": ["url"]
        }}
        ,
        "sort": 
...
def _get_first_data(first_rs):
    ...
    if first_rs:
        for hit in first_rs['hits']['hits']:
            ...
            sql="INSERT INTO es_cyjh(id,timestamp,token,time,title,url,commodity) VALUES(%d,'%s','%s','%s','%s','%s','%s')"%(0,timestamp,token,date,title,url,x)
            try:
                cur.execute(sql)
                db.commit()
            except:
